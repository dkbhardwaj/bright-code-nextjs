...
const TeamGrid: React.FC<CardItem> = ({ data }) => {
  const { sectionPadding, card } = data;
  ...
  const getWidthStyle = () => {
    ...
    if (windowWidth >= 596 && windowWidth < 768) return { width: '50%' };
    if (windowWidth >= 768 && windowWidth < 992) return { width: '33.33%' };
    return {};
  };
      ...
      >
        <div className="wrapper flex w-[calc(100%+2rem)] justify-center flex-wrap ml-[-1rem]">
          {card.map((item, index) => (
            <div
              key={index}
              className="group sm:max-w-[33rem] sm:!w-full mx-[.5rem] mb-[2.5rem]"
              style={getWidthStyle()}
            >
                ...
                {item?.fields?.linkedinUrl && (
                  <div className="profile-icon inline-block duration-300 relative mt-[10px] max-w-[33px] h-[33px]">
                    <Link
                      ...
                      className="redirect absolute inset-0 z-10"
                      target="_blank"
                      rel="noopener noreferrer"
                      aria-label={`${item.fields.title} on LinkedIn`}
                    >
                    ...
                    >
                      <g clipPath="url(#clip0_98_121)">
                        <path
                          className="fill-black duration-300"
